...
def bbox_iou(outputs, targets):
    """
    Calculate the IoU of two sets of boxes.
    We assume that the boxes are in the format (x1, y1, x2, y2).
    Args:
        outputs: (tensor) predicted bounding boxes, size [batch_size, 4]
        targets: (tensor) ground truth bounding boxes, size [batch_size, 4]
    Returns:
        iou (tensor): the computed IoU, size [batch_size]
    """
    # Intersection
    xA = torch.max(outputs[0], targets[0])
    yA = torch.max(outputs[1], targets[1])
    xB = torch.min(outputs[2], targets[2])
    yB = torch.min(outputs[3], targets[3])

    interArea = torch.clamp(xB - xA, min=0) * torch.clamp(yB - yA, min=0)

    # Union
    boxAArea = (outputs[2] - outputs[0]) * (outputs[3] - outputs[1])
    boxBArea = (targets[2] - targets[0]) * (targets[3] - targets[1])

    iou = interArea / (boxAArea + boxBArea - interArea + 1e-6)

    return iou.mean()

# MSE loss for bbox regression
...
class BBoxIoULoss(nn.Module):
    ...
    def forward(self, input, target):
        return 1 - bbox_iou(input, target)
...
class ComboLoss(nn.Module):
    ...
    def forward(self, outputs, targets, bbox_outputs=None, bbox_targets=None):
        loss = 0
        ...
        for k, v in weights.items():
            ...
            if k in self.per_channel:
                ...
                if k in {'bbox_iou', 'bbox_mse'}:
                    val = self.mapping[k](bbox_outputs, bbox_targets)
                else: 
                    val = self.mapping[k](
