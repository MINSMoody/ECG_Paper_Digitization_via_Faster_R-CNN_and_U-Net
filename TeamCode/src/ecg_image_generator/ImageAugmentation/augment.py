...
from TeamCode.src.ecg_image_generator.helper_functions import read_leads, convert_bounding_boxes_to_dict, rotate_bounding_box, get_lead_pixel_coordinate, rotate_points
import numpy as np
...
def get_augment(input_file,output_directory,rotate=25,noise=25,crop=0.01,temperature=6500,bbox=False, store_text_bounding_box=False, json_dict=None):
    ...
    mask_dir = os.path.join(path_dir, 'masks')
    ...
    masks_aug = seq_mask(images=masks)
    ...
    plt.imsave(fname=mask_path,arr=masks_aug[0])
